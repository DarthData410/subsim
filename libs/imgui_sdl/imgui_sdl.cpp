...
namespace {
	...
	namespace TupleHash {
		...
		void CombineHash(std::size_t& seed, const T& value) {
			seed ^= TupleHash::Hash<T>()(value) + 0x9E'37'79'B9 + (seed << 6) + (seed >> 2);
		}
	...
	public:
		...
		bool Contains(const Key& key) const { return Container.find(key) != Container.end(); }
	...
	struct Device final {
		...
		struct TriangleCacheItem final {
		...
		void SetClipRect(const ClipRect& rect) {
			Clip = rect;
                ...
                const SDL_Rect clip_sdl_rect = { rect.X, h - rect.Y - rect.Height, rect.Width, rect.Height };
                // TODO formula is still slightly off
                //SDL_RenderSetClipRect(Renderer, &clip_sdl_rect);
            #else
                const SDL_Rect clip_sdl_rect = { rect.X, rect.Y, rect.Width, rect.Height };
                SDL_RenderSetClipRect(Renderer, &clip_sdl_rect);
            #endif
		}
	...
	void DrawTriangle(ImDrawVert v1, ImDrawVert v2, ImDrawVert v3, SDL_Texture *texture) {
		...
		const Sint32 minXf = SDL_floor(SDL_min(v1.pos.x, SDL_min(v2.pos.x, v3.pos.x))) * 0x10;
		const Sint32 minYf = SDL_floor(SDL_min(v1.pos.y, SDL_min(v2.pos.y, v3.pos.y))) * 0x10;
		const Sint32 maxXf = SDL_ceil(SDL_max(v1.pos.x,  SDL_max(v2.pos.x, v3.pos.x))) * 0x10;
		const Sint32 maxYf = SDL_ceil(SDL_max(v1.pos.y,  SDL_max(v2.pos.y, v3.pos.y))) * 0x10;
		...
		const Sint32 meanXf = round((maxXf - minXf) / 2 / 0x10) * 0x10;
		const Sint32 meanYf = round((maxYf - minYf) / 2 / 0x10) * 0x10;
		...
		const Sint32 a1 = (f2y - f3y) * 0x10;
		const Sint32 a2 = (f3y - f1y) * 0x10;
		const Sint32 a3 = (f1y - f2y) * 0x10;
		const Sint32 b1 = (f3x - f2x) * 0x10;
		const Sint32 b2 = (f1x - f3x) * 0x10;
		const Sint32 b3 = (f2x - f1x) * 0x10;
        ...
        int texture_width = 0, texture_height = 0; // Store texture width and height for use in mapping vertex attributes
		if (texture) {
		...
		const float col1r = Color(v1.col).R * 0xFF / normalization;
		const float col1g = Color(v1.col).G * 0xFF / normalization;
		const float col1b = Color(v1.col).B * 0xFF / normalization;
		const float col1a = Color(v1.col).A * 0xFF / normalization;
		const float col2r = Color(v2.col).R * 0xFF / normalization;
		const float col2g = Color(v2.col).G * 0xFF / normalization;
		const float col2b = Color(v2.col).B * 0xFF / normalization;
		const float col2a = Color(v2.col).A * 0xFF / normalization;
		const float col3r = Color(v3.col).R * 0xFF / normalization;
		const float col3g = Color(v3.col).G * 0xFF / normalization;
		const float col3b = Color(v3.col).B * 0xFF / normalization;
		const float col3a = Color(v3.col).A * 0xFF / normalization;
...
namespace ImGuiSDL {
	...
	static int ImGuiSDLEventWatch(void *userdata, SDL_Event *event) {
		if (event->type == SDL_RENDER_TARGETS_RESET) CurrentDevice->CacheWasInvalidated = true;
		return 0;
	}
	...
	void Initialize(SDL_Renderer* renderer, int windowWidth, int windowHeight) {
		...
		static constexpr uint32_t rmask = 0x00'00'00'FF,
		                          gmask = 0x00'00'FF'00,
		                          bmask = 0x00'FF'00'00,
		                          amask = 0xFF'00'00'00;
		...
		//assert(renderer == nullptr && "ImGuiSDL::Initialize must only be called once.");
	...
	void Render(ImDrawData* drawData) {
		...
		SDL_BlendMode blendMode; // to restore old BlendMode later
		...
		for (unsigned n = 0; n < drawData->CmdListsCount; n++) {
			...
			for (unsigned cmd_i = 0; cmd_i < commandList->CmdBuffer.Size; cmd_i++) {
				...
				if (drawCommand->UserCallback) drawCommand->UserCallback(commandList, drawCommand);
				else {
					// Loops over triangles.
					for (unsigned i = 0; i + 3 <= drawCommand->ElemCount; i += 3) {
						num_triangles++;
                        ...
                        ) continue; // Not in clip rect, ignore
		...
		SDL_SetRenderDrawColor(CurrentDevice->Renderer, initialR, initialG, initialB, initialA);
